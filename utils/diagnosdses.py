        ...
        with col:
            ...
            if filtered_options:
                ...
                for option in filtered_options[:5]:  # Show up to 5 suggestions
                    ...
                    if st.button(f"{option}", key=button_key) and not st.session_state.diagnoses_s1_locked:
    ...
    if st.button("Submit Diagnoses"):
        ...
        if all(diagnosis for diagnosis in diagnoses):
            if len(diagnoses) == len(set(diagnoses)):
                ...
                # Check if diagnoses_s1 is already locked
                if not st.session_state.diagnoses_s1_locked:
                    ...
                    try:
                        ...
                        # Lock the diagnoses_s1
                        st.session_state.diagnoses_s1_locked = True
                    except Exception as e:
                        st.error(f"Error uploading data: {e}")
                else:
                    st.warning("Diagnoses have already been submitted and cannot be changed.")
