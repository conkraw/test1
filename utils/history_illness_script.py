...
def main(db, document_id):
    ...
    if st.session_state.current_page == "historical_features":
        ...
        if st.button("Submit Historical Features"):
            ...
            # Combine entry with session data if needed
            # session_data.update(entry)  # Uncomment if needed for other data
            
            # Ensure entry is correctly populated
            if not entry['assessments']:
                st.error("No assessments found. Please check your entries.")
                return
