...
from utils.firebase_operations import upload_to_firebase, FIREBASE_COLLECTION_NAME  # Import the global variable
        ...
        if unique_code_input:
            try:
                ...
                if unique_code in users['code'].values:
                    ...
                    # Fetch existing session data from Firebase
                    existing_data = db.collection(FIREBASE_COLLECTION_NAME).document(unique_code_input).get()
                    
                    if existing_data.exists:
                        # Load data into session state
                        st.session_state.update(existing_data.to_dict())  # Update session state with existing data
